...
function describeTitle(entry) {
  ...
  switch (action) {
    case "create":
      ...
      if (entity_type === "link")    return t("history.create.link", { name: details.link_name });
      if (entity_type === "owner")   return t("history.create.owner", { name: details.name });
      if (entity_type === "device")  return t("history.create.device", { name: details.name, band: details.band });
      return t("history.create.other", { type: entity_type });

    case "update": {
      if (entity_type === "radio") {
        const { label, siteName } = lookupRadioInfo(entity_id);
        return siteName ? t("history.update.radio", { label, site: siteName }) : t("history.update.radioNoSite", { label });
      }
      ...
      if (entity_type === "owner")   return t("history.update.owner", { name: details.new_name || details.old_name || entity_id });
      if (entity_type === "device")  return t("history.update.device", { name: details.new_name || details.old_name || entity_id });
    ...
    case "delete":
      ...
      if (entity_type === "owner")            return t("history.delete.owner", { name: details.name });
...
function renderHistoryTab() {
  ...
  if (_historyFilter !== "all") {
    entries = entries.filter((e) => {
      ...
      if (_historyFilter === "preferences") return e.entity_type === "owner" || e.entity_type === "device";
